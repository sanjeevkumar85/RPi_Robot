...
;  Global Symbols
;-----------------------------------------------
export  Range_PGA_Start
export _Range_PGA_Start
export  Range_PGA_SetPower
export _Range_PGA_SetPower

export  Range_PGA_SetGain
export _Range_PGA_SetGain

export  Range_PGA_Stop
export _Range_PGA_Stop

IF (Range_PGA_AGNDBUFAPI)
export  Range_PGA_EnableAGNDBuffer
export _Range_PGA_EnableAGNDBuffer
export  Range_PGA_DisableAGNDBuffer
export _Range_PGA_DisableAGNDBuffer
ENDIF
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Range_PGA_Start
;  FUNCTION NAME: Range_PGA_SetPower
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[Range_PGA_GAIN_CR2]                         ; read power value
   and  A, ~POWERMASK                                ; clear power bits in A
   or   A, [X]                                       ; combine power value with balance of reg.
   mov  reg[Range_PGA_GAIN_CR2], A                    ; move complete value back to register
   pop  A
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Range_PGA_SetGain
;
;  DESCRIPTION:
...
;
   ...
   mov  A, reg[Range_PGA_GAIN_CR0]                         ; read mux settings
   and  A, ~GAINREGMASK                                   ; clear gain bits in A
   tst  [X],HIGHGAIN                                      ; See if High Gain is set
   jnz  .SETHIGHGAIN
   and  reg[Range_PGA_GAIN_CR3],~EXGAIN                    ; Clear High Gain bit.
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[Range_PGA_GAIN_CR0], A                         ; move complete value back to register
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
...
.SETHIGHGAIN:
   ...
   or   A, [X]                                            ; combine gain value with balance of reg.
   mov  reg[Range_PGA_GAIN_CR0], A                         ; move complete value back to register
   or   reg[Range_PGA_GAIN_CR3], EXGAIN                    ; Set High Gain bit.
   pop  A
   RAM_EPILOGUE RAM_USE_CLASS_2
   ret
.ENDSECTION
...
_Range_PGA_Stop:
   ...
   and REG[Range_PGA_GAIN_CR2], ~POWERMASK
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Range_PGA_EnableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 Range_PGA_EnableAGNDBuffer:
_Range_PGA_EnableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   and   reg[Range_PGA_GAIN_CR3], ~AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Range_PGA_DisableAGNDBuffer
;
;  DESCRIPTION:
...
;    functions.
;
 Range_PGA_DisableAGNDBuffer:
_Range_PGA_DisableAGNDBuffer:
   RAM_PROLOGUE RAM_USE_CLASS_1
   or    reg[Range_PGA_GAIN_CR3], AGNDBUFMASK
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
.ENDSECTION
ENDIF

; End of File Range_PGA.asm
